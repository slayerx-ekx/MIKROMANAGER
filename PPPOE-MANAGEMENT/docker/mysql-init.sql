...
CREATE TABLE IF NOT EXISTS olts (
    ...
    ip_address VARCHAR(45) NOT NULL,
    snmp_ro VARCHAR(160) NOT NULL,
    snmp_rw VARCHAR(160),
    snmp_port INT DEFAULT 161,
    telnet_host VARCHAR(120),
    telnet_port INT DEFAULT 23,
    telnet_username VARCHAR(120),
    telnet_password VARCHAR(255),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_olts_ip_address (ip_address)
);
...
CREATE TABLE IF NOT EXISTS onu_devices (
    ...
    name VARCHAR(180),
    description TEXT,
    board_port VARCHAR(80),
    status VARCHAR(32),
    rx_power FLOAT,
    tx_power FLOAT,
    last_seen TIMESTAMP NULL,
    FOREIGN KEY (olt_id) REFERENCES olts(id) ON DELETE CASCADE ON UPDATE CASCADE,
    UNIQUE KEY uniq_olt_serial (olt_id, serial_number),
    INDEX idx_onu_serial_number (serial_number),
    INDEX idx_onu_olt_id (olt_id),
    INDEX idx_onu_status (status),
    INDEX idx_onu_last_seen (last_seen)
);
