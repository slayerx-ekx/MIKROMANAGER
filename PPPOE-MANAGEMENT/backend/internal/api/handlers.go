...
func (h *Handler) GetStats(c *gin.Context) {
	// Ambil dari database, bukan live API MikroTik
	stats, err := h.pppRepo.GetStats()
	if err != nil {
		respond(c, 500, false, "Failed to get stats", nil)
		return
	}
...
func (h *Handler) GetPPPSecrets(c *gin.Context) {
	...
	secrets, total, err := h.pppRepo.GetAllSecrets(filter)
	if err != nil {
		respond(c, 500, false, "Failed to get PPP secrets", nil)
		return
	}
	...
	if syncSettings, err := h.syncRepo.GetSettings(); err == nil && syncSettings.LastSyncAt != nil {
		v := formatJakarta(*syncSettings.LastSyncAt)
		lastSyncAt = &v
	}
...
func (h *Handler) GetRouterStatuses(c *gin.Context) {
	// Ambil dari database, bukan live API MikroTik
	statuses, err := h.pppRepo.GetRouterStatuses()
	if err != nil {
		respond(c, 500, false, "Failed to get router statuses", nil)
		return
	}
...
func (h *Handler) GetChartData(c *gin.Context) {
	// Ambil dari database, bukan live API MikroTik
	data, err := h.pppRepo.GetOnlineChartData()
	if err != nil {
		respond(c, 500, false, "Failed to get chart data", nil)
		return
	}
	respond(c, 200, true, "OK", data)
}

// RefreshPPPoEData - trigger ambil data baru dari MikroTik, simpan ke database, lalu return data terbaru
func (h *Handler) RefreshPPPoEData(c *gin.Context) {
	routerIDStr := strings.TrimSpace(c.Query("router_id"))

	var routers []models.Router
	var err error

	if routerIDStr != "" {
		routerID, convErr := strconv.Atoi(routerIDStr)
		if convErr != nil || routerID <= 0 {
			respond(c, 400, false, "Invalid router_id", nil)
			return
		}
		router, getErr := h.routerRepo.GetByID(routerID)
		if getErr != nil {
			respond(c, 404, false, "Router not found", nil)
			return
		}
		routers = []models.Router{*router}
	} else {
		all, getAllErr := h.routerRepo.GetAll()
		if getAllErr != nil {
			respond(c, 500, false, "Failed to get routers", nil)
			return
		}
		for _, r := range all {
			if r.IsActive {
	...
	if len(routers) == 0 {
		respond(c, 200, true, "Tidak ada router aktif", gin.H{"synced": 0})
		return
	}

	// Sync setiap router: ambil dari MikroTik, simpan ke database
	synced := 0
	failed := 0
	for _, router := range routers {
		r := router
		if err = h.syncSvc.SyncRouter(&r); err != nil {
			failed++
		} else {
			synced++
		}
	...
	if synced > 0 {
		h.syncRepo.UpdateLastSync()
	}

	msg := fmt.Sprintf("Refresh selesai: %d router berhasil", synced)
	if failed > 0 {
		msg += fmt.Sprintf(", %d gagal", failed)
	}

	respond(c, 200, synced > 0, msg, gin.H{
		"synced": synced,
		"failed": failed,
	})
}
