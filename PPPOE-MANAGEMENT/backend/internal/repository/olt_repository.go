...
func (r *OLTRepository) GetByID(id uint) (*model.OLT, error) {
	var olt model.OLT
...
func (r *OLTRepository) Delete(id uint) error {
	return r.db.Delete(&model.OLT{}, id).Error
}

func (r *OLTRepository) UpsertONUs(oltID uint, onus []model.ONUDevice) error {
	if len(onus) == 0 {
		return nil
	}
	for i := range onus {
...
func (r *OLTRepository) ListONUs(oltID uint) ([]model.ONUDevice, error) {
