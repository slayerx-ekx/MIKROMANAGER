...
func (s *SyncService) SyncRouter(router *models.Router) error {
	...
	// Build db records — merge secrets with active status
	...
	for _, sec := range secrets {
		s := models.PPPSecret{
			...
			InterfaceName: firstFilled(
				ifaceByUser[sec.Name]["interface"],
				ifaceByUser[sec.Name]["actual-interface"],
				ifaceByUser[sec.Name]["running-on"],
				ifaceByUser[sec.Name]["parent-interface"],
				ifaceByUser[sec.Name]["master-interface"],
			),
			Profile:       sec.Profile,
			Service:       sec.Service,
			...
			LastLinkDown: firstFilled(
				ifaceByUser[sec.Name]["last-link-down-time"],
				ifaceByUser[sec.Name]["last-link-down"],
				ifaceByUser[sec.Name]["last-disconnected-time"],
				ifaceByUser[sec.Name]["last-logged-out"],
				ifaceByUser[sec.Name]["last-logout"],
			),
			LastLinkUp: firstFilled(
				ifaceByUser[sec.Name]["last-link-up-time"],
				ifaceByUser[sec.Name]["last-link-up"],
				ifaceByUser[sec.Name]["last-logged-in-time"],
				ifaceByUser[sec.Name]["last-logged-in"],
				ifaceByUser[sec.Name]["last-login"],
			),
			Status:        "offline",
		}
		if ac, ok := activeMap[sec.Name]; ok {
			s.Status = "online"
			s.Uptime = ac.Uptime
