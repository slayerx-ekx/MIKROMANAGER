...
type OLTService struct {
	...
	syncState       map[int]model.OLTSyncStatus
	lockMu          sync.Mutex
	oltLocks        map[int]*sync.Mutex
}
...
func NewOLTService(repo *repository.OLTRepository, troubleshootRepo *repository.TroubleshootRepository, snmpSvc *oltsnmp.Service, logger *logrus.Logger, pollInterval time.Duration) *OLTService {
	...
	svc := &OLTService{
		...
		syncState:        make(map[int]model.OLTSyncStatus),
		oltLocks:         make(map[int]*sync.Mutex),
	}
...
func (s *OLTService) GetOLT(id int) (*model.OLT, error) {
	return s.repo.GetByID(id)
}

func (s *OLTService) UpdateOLT(id int, olt *model.OLT) error {
	normalizeOLT(olt)
	if err := validateOLT(*olt); err != nil {
		return err
	}
...
func (s *OLTService) DeleteOLT(id int) error {
...
func (s *OLTService) SyncONUs(oltID int) ([]model.ONUDevice, error) {
...
func (s *OLTService) StartONUSync(oltID int) (model.OLTSyncStatus, error) {
...
func (s *OLTService) GetONUSyncStatus(oltID int) model.OLTSyncStatus {
	s.stateMu.RLock()
...
func (s *OLTService) ListONUs(oltID int, refresh bool) ([]model.ONUDevice, error) {
	if refresh {
...
func (s *OLTService) GetRawTelnetDump(oltID int, mode string) (*model.OLTTelnetDump, error) {
	olt, err := s.repo.GetByID(oltID)
	if err != nil {
		return nil, err
	}
...
func (s *OLTService) RunTelnetCommand(oltID int, preset, target string) (*model.OLTTelnetDump, error) {
	olt, err := s.repo.GetByID(oltID)
	if err != nil {
		return nil, err
	}
...
func (s *OLTService) syncLock(oltID int) *sync.Mutex {
	s.lockMu.Lock()
