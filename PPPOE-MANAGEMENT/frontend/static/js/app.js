...
async function loadPPP(page) {
  ...
  var d = r.data || r;
  var lastSyncAt = r.last_sync_at || d.last_sync_at;
  ...
  var totalEl = document.getElementById('ppp-total') || document.getElementById('ppp-total-info');
  if (totalEl) totalEl.textContent = 'Total: ' + (d.total || 0).toLocaleString();

  if (!d.data || !d.data.length) {
    tbody.innerHTML = '<tr><td colspan="12" class="empty-state">Tidak ada data. Lakukan refresh dahulu.</td></tr>';
    var piEl = document.getElementById('ppp-info') || document.getElementById('ppp-page-info');
    if (piEl) piEl.textContent = '';
    var pgEl = document.getElementById('ppp-pages') || document.getElementById('ppp-pagination');
  ...
  for (var i = 0; i < d.data.length; i++) {
    ...
    if (isIsolir) {
      statusBadge += '&nbsp;<span class="badge badge-isolir">ISOLIR</span>';
    }

    // IP & Uptime hanya untuk online
    var ipTxt = (isOnline && u.ip_address)
      ? '<span style="color:#fb923c;font-family:monospace;font-size:12px">' + u.ip_address + '</span>'
      : '<span style="color:#475569">-</span>';
    var uptimeTxt = (isOnline && u.uptime) ? u.uptime : '-';

    // Last link down & up — tampilkan untuk SEMUA status jika ada data
    var linkDownTxt = u.last_link_down
      ? '<span style="color:#f87171;font-family:monospace;font-size:11px">' + fmtDate(u.last_link_down) + '</span>'
      : '<span style="color:#475569">-</span>';
    var linkUpTxt = u.last_link_up
      ? '<span style="color:#34d399;font-family:monospace;font-size:11px">' + fmtDate(u.last_link_up) + '</span>'
      : '<span style="color:#475569">-</span>';

    // Caller ID / MAC
    var callerTxt = u.caller_id
      ? '<span style="font-family:monospace;font-size:11px;color:#94a3b8">' + u.caller_id + '</span>'
      : '<span style="color:#475569">-</span>';

    // Router info
    var routerTxt = (u.router_name || '-');
    var routerIpTxt = u.router_ip
      ? '<a href="http://' + u.router_ip + '" target="_blank" class="link-orange" style="font-size:11px">' + u.router_ip + '</a>'
      : '';

    // Action buttons
    var pingBtn = (isOnline && u.ip_address)
    ...
    var editBtn = '<button class="btn btn-sm" style="background:#3b82f6;color:#fff;margin-right:2px" onclick="openPPPUserModal(' + (u.router_id || 0) + ',\'' + (u.username || '') + '\')">Edit</button>';
    var delBtn = '<button class="btn btn-sm" style="background:#ef4444;color:#fff" onclick="deletePPPUser(' + (u.router_id || 0) + ',\'' + (u.username || '') + '\')">Hapus</button>';

    rows += '<tr>';
    rows += '<td style="color:#64748b">' + (start + i) + '</td>';
    rows += '<td style="font-weight:500;color:#f1f5f9">' + (u.username || '-') + '</td>';
    rows += '<td>' + ipTxt + '</td>';
    rows += '<td style="font-family:monospace;font-size:12px;color:#94a3b8">' + uptimeTxt + '</td>';
    rows += '<td style="color:#cbd5e1">' + routerTxt + (routerIpTxt ? '<br>' + routerIpTxt : '') + '</td>';
    rows += '<td style="font-size:12px;color:#94a3b8">' + (u.profile || '-') + '</td>';
    rows += '<td style="font-size:12px;color:#64748b">' + (u.service || 'pppoe') + '</td>';
    rows += '<td>' + callerTxt + '</td>';
    rows += '<td>' + linkDownTxt + '</td>';
    rows += '<td>' + linkUpTxt + '</td>';
    rows += '<td>' + statusBadge + '</td>';
    rows += '<td><div class="gap-row">' + pingBtn + editBtn + delBtn + '</div></td>';
    rows += '</tr>';
  }
  tbody.innerHTML = rows;

  var end = Math.min(start + d.limit - 1, d.total);
  var piEl2 = document.getElementById('ppp-info') || document.getElementById('ppp-page-info');
  if (piEl2) piEl2.textContent = start + '\u2013' + end + ' of ' + d.total.toLocaleString();
  var pgEl2 = document.getElementById('ppp-pages') || document.getElementById('ppp-pagination');
  if (pgEl2) renderPagination(pgEl2.id, d.total_pages, d.page, loadPPP);
}
...
async function refreshPPPoEData() {
  ...
  var routerId = document.getElementById('f-router').value || '';
  
  if (btn) btn.disabled = true;
  if (icon) icon.className = 'fas fa-spinner fa-spin';
  
  var url = '/pppoe/refresh';
  if (routerId) {
  ...
  if (r && r.success) {
    showToast('Data PPPoE berhasil diperbarui!');
    loadPPP(1);
    loadStats();
  } else {
    showToast('Gagal memperbarui data PPPoE: ' + (r ? r.message : ''), 'error');
  }
