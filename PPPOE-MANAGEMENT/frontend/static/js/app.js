...
// ===== PPP MONITORING =====
var _pppPollingEnabled = false; // tracking apakah auto_sync aktif
...
async function loadPPP(page) {
  ...
  if (!r || !r.success) {
    tbody.innerHTML = '<tr><td colspan="9" class="empty-state" style="color:#f87171">Gagal memuat data. Pastikan sudah ada data di database (klik Refresh).</td></tr>'; return;
  }
  var d = r.data;

  // Tampilkan info last sync
  var lastSyncEl = document.getElementById('ppp-last-sync');
  if (lastSyncEl) {
    if (r.data && r.last_sync_at) {
      lastSyncEl.textContent = '🕐 Diperbarui: ' + fmtDate(r.last_sync_at);
    } else if (d && d.last_sync_at) {
      lastSyncEl.textContent = '🕐 Diperbarui: ' + fmtDate(d.last_sync_at);
    } else {
      lastSyncEl.textContent = '🕐 Belum pernah diperbarui — klik Refresh';
    }
  }

  // d bisa object atau langsung PaginatedResponse, handle kedua format
  var items = d.data || d;
  var total = d.total !== undefined ? d.total : (Array.isArray(items) ? items.length : 0);
  var currentPage = d.page || page;
  var limit = d.limit || 25;
  var totalPages = d.total_pages || 1;
  var lastSyncAt = d.last_sync_at || r.last_sync_at;

  // Update last sync juga dari data object
  if (lastSyncEl && lastSyncAt) {
    lastSyncEl.textContent = '🕐 Diperbarui: ' + fmtDate(lastSyncAt);
  }

  document.getElementById('ppp-total').textContent = 'Total: ' + (total || 0).toLocaleString();
  if (!items || !items.length) {
    tbody.innerHTML = '<tr><td colspan="9" class="empty-state">Tidak ada data. Klik tombol Refresh untuk mengambil data dari MikroTik.</td></tr>';
    document.getElementById('ppp-page-info') && (document.getElementById('ppp-page-info').textContent = '');
    document.getElementById('ppp-info') && (document.getElementById('ppp-info').textContent = '');
    document.getElementById('ppp-pagination') && (document.getElementById('ppp-pagination').innerHTML = '');
    document.getElementById('ppp-pages') && (document.getElementById('ppp-pages').innerHTML = '');
    return;
  }
  ...
  document.getElementById('ppp-page-info') && (document.getElementById('ppp-page-info').textContent = start + '–' + end + ' of ' + total.toLocaleString());
  document.getElementById('ppp-info') && (document.getElementById('ppp-info').textContent = start + '–' + end + ' of ' + total.toLocaleString());
  renderPagination('ppp-pagination', totalPages, currentPage, loadPPP);
  renderPagination('ppp-pages', totalPages, currentPage, loadPPP);
}

// ===== REFRESH PPPOE DARI MIKROTIK =====
async function refreshPPPoEData() {
  var btn = document.getElementById('ppp-refresh-btn');
  var icon = document.getElementById('ppp-refresh-icon');
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24" style="width:14px;height:14px;animation:spin 1s linear infinite"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg> Mengambil data...';
  }

  var routerId = document.getElementById('f-router') ? document.getElementById('f-router').value : '';
  var url = '/pppoe/refresh' + (routerId ? '?router_id=' + routerId : '');

  showToast('Mengambil data dari MikroTik, mohon tunggu...', 'info');

  var r = await api(url, { method: 'POST' });

  if (btn) {
    btn.disabled = false;
    btn.innerHTML = '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24" style="width:14px;height:14px"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg> Refresh';
  }

  if (r && r.success) {
    showToast(r.message || 'Data berhasil diperbarui dari MikroTik!');
    loadPPP(1);
    // Update dashboard juga
    if (document.getElementById('stat-total')) loadStats();
    if (document.getElementById('router-status-list')) loadRouterStatus();
  } else {
    showToast((r && r.message) || 'Gagal mengambil data dari MikroTik', 'error');
  }
}
...
async function loadSyncPage() {
  ...
  if (!r || !r.success) return;
  var cb = document.getElementById('sync-cb');
  if (cb) cb.checked = r.data.auto_sync_enabled;
  var iv = document.getElementById('sync-interval');
  if (iv) iv.value = r.data.sync_interval_seconds;

  // Update settings badge
  var badge = document.getElementById('sync-settings-badge');
  if (badge) {
    badge.textContent = r.data.auto_sync_enabled ? 'Auto ON' : 'Manual';
    badge.className = 'badge ' + (r.data.auto_sync_enabled ? 'b-admin' : 'b-view');
  }

  var sr = await api('/sync/status');
  if (sr && sr.success) {
    var sd = sr.data;
    var statusEl = document.getElementById('sync-status-disp');
    var lastEl = document.getElementById('sync-last-disp');
    if (statusEl) statusEl.textContent = sd.auto_sync_enabled ? ('Auto (setiap ' + sd.sync_interval_seconds + 'd)') : 'Manual';
    if (lastEl) lastEl.textContent = fmtDate(sd.last_sync_at);
  }
}

async function saveSyncSettings() {
  var data = {
    auto_sync_enabled: document.getElementById('sync-cb').checked,
  ...
  if (r && r.success) {
    ...
    loadSyncBadge();
    // Update polling state berdasarkan setting baru
    _pppPollingEnabled = data.auto_sync_enabled;
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Polling dashboard hanya aktif jika auto_sync enabled di settings
  // Cek setting dari server, lalu mulai interval jika perlu
  api('/sync/status').then(function(r) {
    if (r && r.success && r.data && r.data.auto_sync_enabled) {
      _pppPollingEnabled = true;
      var intervalMs = ((r.data.sync_interval_seconds || 60) * 1000);
      // Minimal 30 detik untuk dashboard polling
      var pollMs = Math.max(30000, intervalMs);
      setInterval(function() {
        if (!_pppPollingEnabled) return;
        var active = document.querySelector('.nav-link.active');
        var page = active ? active.id.replace('nav-','') : '';
        if (page === 'dashboard') { loadStats(); loadSyncBadge(); }
      }, pollMs);
    }
    // Jika auto_sync disabled, tidak ada polling otomatis — user harus klik Refresh
  });
});
